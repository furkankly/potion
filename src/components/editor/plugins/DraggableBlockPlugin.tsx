...
function useDraggableBlockMenu(
  ...
  return createPortal(
    <>
      <div
        className="draggable-block-menu absolute left-0 top-0 cursor-pointer opacity-0 will-change-transform active:cursor-pointer"
        ref={menuRef}
        draggable={true}
      ...
      >
        {isEditable ? (
          <>
            <div className="relative h-6 w-6 bg-[url('../images/drag.svg')] bg-cover opacity-50 active:cursor-pointer" />
            <div className="absolute h-6 w-6 -translate-y-6 -translate-x-4 font-bold text-white">
              {draggableBlockElem?.dataset.nodeKey}
            </div>
          </>
        ) : null}
      </div>
      <div
        className="pointer-events-none absolute left-0 top-0 h-1 w-3/4 bg-rose-100 opacity-0 will-change-transform"
        ref={targetLineRef}
      />
    </>,
    anchorElem
